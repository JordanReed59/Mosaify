...
resource "aws_api_gateway_integration" "option_post_lambda" {
  rest_api_id = "${aws_api_gateway_rest_api.gateway.id}"
  resource_id = "${aws_api_gateway_method.option_post_method.resource_id}"
...
resource "aws_lambda_permission" "apigw_option_lambda" {
  ...
  source_arn = "arn:aws:execute-api:${var.region}:${local.account_id}:${aws_api_gateway_rest_api.gateway.id}/*/${aws_api_gateway_method.option_post_method.http_method}${aws_api_gateway_resource.option_method_resource.path}"
}
################ Option resource ################
